...
def point_mass_maze(direction=RIGHT, length=1.2, borders=True):
    ...
    particle.geom(name='particle_geom', type='sphere', size='0.03', rgba='1.0 1.0 0.0 1', contype=1)
    particle.site(name='particle_site', pos=[0,0,0], size=0.01)
    particle.joint(name='ball_x', type='slide', pos=[0,0,0], axis=[1,0,0])
    particle.joint(name='ball_y', type='slide', pos=[0,0,0], axis=[0,1,0])

    target = worldbody.body(name='target', pos=[length/2,length-0.1,0])
    target.geom(name='target_geom', conaffinity=2, type='sphere', size=0.03, rgba=[0,0.9,0.1,1])

    L = -0.1
    R = length
    U = length
    D = -0.1
    #if borders:0.9 0.4 0.6 1
    if borders:
        worldbody.geom(conaffinity=1, fromto=[L, D, .01, R, D, .01], name="sideS", rgba="1.0 0.0 1.0 1", size=".02", type="capsule")
        worldbody.geom(conaffinity=1, fromto=[R, D, .01, R, U, .01], name="sideE", rgba="1.0 0.0 1.0 1", size=".02", type="capsule")
        worldbody.geom(conaffinity=1, fromto=[L, U, .01, R, U, .01], name="sideN", rgba="1.0 0.0 1.0 1", size=".02", type="capsule")
        worldbody.geom(conaffinity=1, fromto=[L, D, .01, L, U, .01], name="sideW", rgba="1.0 0.0 1.0 1", size=".02", type="capsule")
    ...
    worldbody.geom(conaffinity=1, fromto=[BL, BH, .01, BR, BH, .01], name="barrier", rgba="1.0 0.0 1.0 1", size=".02", type="capsule")
